...
Gibbs_method <- function(y, lambdas=lambdas, n_lambda=n_lambda, Theta = Theta, combination = combination, max.elongation = em.iter, em.tol=em.tol){
  ...
  calculate.R.internal = function(y, theta=NULL, lower_upper=NULL, gibbs.iter=1000, mc.iter=1000, verbose = TRUE)
  {
  ...
  calculate_EM_Gibbs = function(combination, y, lambda1 = lambdas[1,1], lambda2 = lambdas[1,2], Theta=NULL, lower_upper, em.tol=.001, em.iter=10, c.em.iter=1, gibbs.iter=1000, mc.iter=1000)
  {
    p = ncol(y[[1]]) 
    c.em.iter = 1
    K <- length(y)
    ...
    {   
      for(k in 1:K){
        R[[k]]   <- calculate.R.internal(y[[k]], theta = Theta[[k]], lower_upper = lower_upper[[k]], gibbs.iter = gibbs.iter, mc.iter = mc.iter)
      }
  ...
  Gibbs.method <- calculate_EM_Gibbs(combination, y, lambda1 = lambdas[combination,1], lambda2 = lambdas[combination,2], Theta=Theta, lower_upper = lower_upper, em.tol = em.tol, em.iter = max.elongation, gibbs.iter = 500, mc.iter = 1000)
